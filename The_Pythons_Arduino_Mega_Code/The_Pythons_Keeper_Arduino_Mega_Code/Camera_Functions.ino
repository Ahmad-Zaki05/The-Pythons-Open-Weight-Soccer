...
void new_new_read_Camera() {
  ...
  if (cam_angle_str[0] == 'S') { // Searching for ball
    ...
    //    Serial.println("Tracking Ball");
    //Serial.println(cam_angle);
    forward = 40 * sin((float)cam_angle * M_PI / 180);
    side = -40 * cos((float)cam_angle * M_PI / 180);
    //Serial.print(forward);
    //Serial.print("\t");
    //Serial.println(side);
  }
    ...
    if (cam_angle_str[2] != 's') {
      ...
      if (cos((float)cam_angle * M_PI / 180) < -0.1) side = max(45, -70 * cos((float)cam_angle * M_PI / 180));// 30 // max(40, -80 * cos((float)cam_angle * M_PI / 180))
      else if (cos((float)cam_angle * M_PI / 180) > 0.1) side = min(-45, -70 * cos((float)cam_angle * M_PI / 180));//-30 // min(-40, -80 * cos((float)cam_angle * M_PI / 180))
    }
    Serial.println("Defending");
    //Serial.println(cam_angle);
